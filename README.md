...
- **Robot hand data** (non-public). We release code/protocols and model weights, but **no raw data**.  
...
The Robot hand data we use is non-public. We release split manifests (Packs A/B), preprocessing interfaces, configs, and checkpoints.

For authorized users with access to the raw dataset:

```bash
export DLR_HAND_ROOT="/secure/location/ROBOT_HAND"   # your private path

# Pack A (seed=0) — budgets: 1,10,25,100
python -m data.make_splits "$ROBOT_HAND_ROOT" configs/splits --pack A --seed 0 --budgets 1 10 25 100

# Pack B (seed=1) — anchors: 25,100
python -m data.make_splits "$ROBOT_HAND_ROOT" configs/splits --pack B --seed 1 --budgets 25 100
...
- Robot hand data are not redistributed. Only split manifests and preprocessing interfaces are provided.  
